#============================================================
# OMEGA QUANTUM AI V31.0 - AUTO-BOOT EDITION
# 7-LAYER ENSEMBLE + GENETIC OPTIMIZER + LIVE SYNC
...
def hurst_exponent(seq):
    ...
    n_pts = len(rs_list)
    sx = sum(p[0] for p in rs_list); sy = sum(p[1] for p in rs_list)
    sxy = sum(p[0]*p[1] for p in rs_list); sxx = sum(p[0]**2 for p in rs_list)
...
class FractalMarkovEngine:
    def __init__(self):
        self.m5 = defaultdict(lambda: [0, 0])
        self.m4 = defaultdict(lambda: [0, 0])
        self.m3 = defaultdict(lambda: [0, 0])
        self.hurst = 0.5
        self.trained = False
    ...
    def train(self, dataset):
        ...
        self.m5.clear(); self.m4.clear(); self.m3.clear()
        for i in range(n - 5): self.m5[tuple(seq[i:i+5])][seq[i+5]] += 1
        for i in range(n - 4): self.m4[tuple(seq[i:i+4])][seq[i+4]] += 1
        for i in range(n - 3): self.m3[tuple(seq[i:i+3])][seq[i+3]] += 1
    ...
    def predict(self, dataset):
        ...
        votes, weights = [], []
        if len(seq) >= 5:
            c = self.m5.get(tuple(seq[-5:]))
            if c and sum(c) >= 2: votes.append((c[1]+1)/(sum(c)+2)); weights.append(0.35)
        if len(seq) >= 4:
            c = self.m4.get(tuple(seq[-4:]))
            if c and sum(c) >= 2: votes.append((c[1]+1)/(sum(c)+2)); weights.append(0.25)
        if len(seq) >= 3:
            c = self.m3.get(tuple(seq[-3:]))
            if c and sum(c) >= 2: votes.append((c[1]+1)/(sum(c)+2)); weights.append(0.15)
        if self.hurst > 0.6: votes.append(0.8 if seq[-1] == 1 else 0.2); weights.append(0.15)
        elif self.hurst < 0.4: votes.append(0.2 if seq[-1] == 1 else 0.8); weights.append(0.10)
        if not votes: return "BIG" if seq[-1] == 1 else "SMALL", 55
        tw = sum(weights); wp = sum(v*w for v,w in zip(votes, weights)) / tw
        pred = "BIG" if wp >= 0.5 else "SMALL"
        conf = int(clamp(65 + abs(wp - 0.5) * 100, 55, 95))
        return pred, conf
...
class RussianEngineV3:
    ...
    def train(self, dataset):
        ...
        for i in range(n - 1): self.trans[seq[i]][seq[i+1]] += 1
        self.trained = True
    ...
    def predict(self, dataset):
        ...
        seq = [1 if str(d.get("result", "")).upper() == "BIG" else 0 for d in dataset]
        last = seq[-1]
        tb = self.trans[last].get(1, 0) + 1; ts = self.trans[last].get(0, 0) + 1
        mp = tb / (tb + ts)
        if self.alt_score > 0.6: ap = 0.2 if last == 1 else 0.8
        elif self.alt_score < 0.4: ap = 0.8 if last == 1 else 0.2
        else: ap = 0.5
        fp = (mp * 0.6) + (ap * 0.2) + (self.base_rate * 0.2)
        pred = "BIG" if fp >= 0.5 else "SMALL"
        conf = int(clamp(60 + abs(fp - 0.5) * 80, 55, 95))
        return pred, conf

#============================================================
# 🕵️ MODEL 3: HMM SIMULATOR
#============================================================
class HMMSimulator:
    def __init__(self):
        self.tp = [[0.7, 0.3], [0.3, 0.7]]
        self.trained = False
    ...
    def train(self, dataset):
        ...
        self.tp = [[c00/s0, c01/s0], [c10/s1, c11/s1]]
        self.trained = True
    ...
    def predict(self, dataset):
        ...
        seq = [1 if str(d.get("result", "")).upper() == "BIG" else 0 for d in dataset]
        last = seq[-1]
        p0 = self.tp[last][0]; p1 = self.tp[last][1]
        pred = "BIG" if p1 >= p0 else "SMALL"
        conf = int(clamp(60 + abs(p1 - p0) * 60, 55, 90))
        return pred, conf

#============================================================
# 🧬 MODEL 4: PURE PYTHON MLP
#============================================================
class PureMLP:
    def __init__(self, input_size=15, hidden_size=30):
        ...
        self.b2 = 0.0; self.lr = 0.05; self.trained = False
    ...
    def predict(self, dataset):
        ...
        conf = int(clamp(60 + abs(prob - 0.5) * 80, 55, 92))
        return pred, conf
...
class HarmonicDFTEngine:
    def __init__(self):
        self.period = 1; self.trained = False

    def train(self, dataset):
        seq = [1 if str(d.get("result", "")).upper() == "BIG" else 0 for d in dataset]
        n = len(seq)
        if n < 30: self.trained = False; return
        mean = sum(seq) / n; centered = [x - mean for x in seq]
        max_power = -1; best_period = 1
        for k in range(2, min(50, n // 2)):
            real, imag = 0.0, 0.0
            for t in range(n):
                angle = 2 * math.pi * k * t / n
                real += centered[t] * math.cos(angle); imag -= centered[t] * math.sin(angle)
            power = real**2 + imag**2
            if power > max_power: max_power = power; best_period = k
        self.period = best_period; self.trained = True

    def predict(self, dataset):
        if not self.trained or len(dataset) < self.period: return None, 0
        seq = [1 if str(d.get("result", "")).upper() == "BIG" else 0 for d in dataset]
        idx = len(seq) - self.period
        if idx >= 0:
            pred = "BIG" if seq[idx] == 1 else "SMALL"
            return pred, 65
        return "BIG", 50
...
class SequenceKNN:
    def __init__(self, k=5, window=10):
        self.k = k; self.window = window; self.memory = []; self.trained = False
    ...
    def predict(self, dataset):
        ...
        conf = int(clamp(60 + (votes.most_common(1)[0][1] / self.k) * 40, 55, 88))
        return pred, conf
...
class MonteCarloSimulator:
    ...
    def train(self, dataset):
        ...
        s0 = c00 + c01; s1 = c10 + c11
        self.tm = [[c00/s0, c01/s0], [c10/s1, c11/s1]]
        self.trained = True

    def predict(self, dataset):
        if not self.trained or len(dataset) < 2: return None, 0
        seq = [1 if str(d.get("result", "")).upper() == "BIG" else 0 for d in dataset]
        last = seq[-1]; big_wins = 0
        for _ in range(self.simulations):
            curr = last
            for _ in range(5):
                r = random.random()
                if curr == 0: curr = 1 if r < self.tm[0][1] else 0
                else: curr = 0 if r < self.tm[1][0] else 1
            if curr == 1: big_wins += 1
        prob = big_wins / self.simulations
        pred = "BIG" if prob >= 0.5 else "SMALL"
        conf = int(clamp(60 + abs(prob - 0.5) * 80, 55, 90))
        return pred, conf

#============================================================
# 🧬 GENETIC ENSEMBLE CORE
#============================================================
class GeneticEnsembleCore:
    ...
    def train_all(self, dataset):
        for name, model in self.models.items():
            try:
                if name == "MLP": model.train_batch(dataset, epochs=2)
                else: model.train(dataset)
            except Exception as e:
                print(f"⚠️ {name} train error: {e}")

    def record(self, model_name, is_win):
        self.perf[model_name].append(1 if is_win else 0)
        if len(self.perf[model_name]) > 50: self.perf[model_name].pop(0)

    def evolve(self):
        self.gen += 1
        if self.gen % 10 != 0: return
        fitness = {}; total = 0
        for name in self.models:
            hist = self.perf[name]
            if len(hist) >= 5: fitness[name] = max(0.05, sum(hist) / len(hist))
            else: fitness[name] = 0.1
            total += fitness[name]
        for name in self.models:
            self.weights[name] = fitness[name] / total if total > 0 else 1.0 / len(self.models)
    ...
    def predict(self, dataset):
        ...
        if not votes: return None, 0, {}
        wp = 0.0; tw = 0.0; details = {}
        for name in votes:
            w = self.weights.get(name, 0.1)
            wp += votes[name] * w; tw += w
            details[name] = {"pred": "BIG" if votes[name] == 1.0 else "SMALL", "conf": int(confs[name]*100), "weight": round(w, 2)}
        if tw > 0: wp /= tw
        final_pred = "BIG" if wp >= 0.5 else "SMALL"
        final_conf = int(clamp(70 + abs(wp - 0.5) * 40, 65, 99))
...
class TimeRiskIntelligence:
    def __init__(self):
        ...
        self.capital = 1000.0; self.max_dd = 0; self.dd = 0

    def record(self, is_win, dt, conf):
        session = self._session(dt.hour)
        ...
        if is_win:
            ...
            self.capital += (conf / 100.0) * 10
        else:
            self.capital -= 10; self.dd += 10
        if self.dd > self.max_dd: self.max_dd = self.dd
        if is_win: self.dd = 0
    ...
    def get_report(self):
        ...
        return best_session, best_rate * 100, self.capital, self.max_dd
...
def get_main_keyboard():
    return {
        "inline_keyboard": [
            [{"text": "📊 পরিসংখ্যান", "callback_data": "btn_stats"}, {"text": "🕒 সেরা টাইম", "callback_data": "btn_time"}],
            [{"text": "🧬 AI মডেল", "callback_data": "btn_models"}, {"text": "🎯 সিগন্যাল", "callback_data": "btn_signal"}],
            [{"text": "💰 রিস্ক", "callback_data": "btn_risk"}, {"text": "🧠 জেনেটিক", "callback_data": "btn_genetic"}]
        ]
    }
...
class OmegaEngine:
    ...
    def sync_live(self):
        """Fetches current live period from API and syncs bot state."""
        try:
            resp = requests.get(API_URL, params={"pageNo": 1, "pageSize": 2, "t": int(time.time()*1000)}, timeout=8)
            if resp.status_code == 200:
                ...
                if lst:
                    ...
                    print(f"✅ Live Synced: Period {live_issue}")
                    return True
        except Exception as e:
    ...
    def load_and_train(self, content):
        """Loads dataset, trains all models, syncs live, and broadcasts first signal."""
        records = self.parse_csv(content)
        if len(records) < 50:
            print(f"⚠️ Not enough data: {len(records)} records (need 50+)")
            return False, 0
        ...
        # CRITICAL: Sync to live market
        if self.sync_live():
            next_issue = str(int(self.last_issue) + 1)
            now = datetime.now(BD_TZ)
            msg = (
                f"👑 <b>OMEGA QUANTUM AI V31.0 INITIATED</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
            ...
            print(f"✅ First signal broadcasted for Period {next_issue}")
            return True, len(records)
        else:
            print("⚠️ Live sync failed")
            return False, 0

    def run(self):
        print("🚀 OMEGA QUANTUM AI V31.0 STARTED...")
        offset = None
        
        # 🔥 AUTO-BOOT: Load dataset.csv from same directory
        if os.path.exists(DATASET_FILE):
            print(f"📂 Found {DATASET_FILE}. Auto-booting...")
            try:
                with open(DATASET_FILE, "r", encoding="utf-8") as f:
                    content = f.read()
                ok, cnt = self.load_and_train(content)
                if ok:
                    print(f"✅ Auto-boot successful! Trained on {cnt} records.")
                else:
                    print("⚠️ Auto-boot failed. Waiting for Telegram file...")
            except Exception as e:
                print(f"⚠️ Auto-boot error: {e}")
        else:
            print(f"📂 {DATASET_FILE} not found. Waiting for Telegram file...")
        ...
        while not _shutdown.is_set():
            ...
            for u in updates:
                ...
                if text == "/start":
                    self.bot.send_message(chat_id, "👑 <b>Omega Quantum AI V31.0</b>\n\nCSV ফাইল সেন্ড করুন।", reply_markup=kb)
            ...
            # Live Loop
            try:
                resp = requests.get(API_URL, params={"pageNo": 1, "pageSize": 10, "t": int(time.time()*1000)}, timeout=8)
                if resp.status_code == 200:
                    lst = resp.json().get("data", {}).get("list", [])
                    if lst:
                        ...
                        if curr_issue != self.last_issue:
                            ...
                            if self.last_pred:
                                ...
                                for m_name, m_data in self.last_details.items():
                                    ...
                                    self.core.record(m_name, m_is_win)
    ...
    def _gen_risk(self):
        cap, dd = self.risk.capital, self.risk.max_dd
    ...
    def _gen_genetic(self):
        ...
        msg += f"🧬 Generation: <code>{self.core.gen}</code>\n\n"
...
def run_web():
    class S(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            self.end_headers()
            self.wfile.write(b'{"status":"Omega V31.0 Running"}')
